...
function renderStage(){
  ...
  questions.forEach((q,i)=>{
    ...
    card.innerHTML = `<div class='qtext'><strong>Q<span class="qnum"></span>.</strong> ${q.q}</div>`;
...
function showQuestion(position){
  ...
  setTimeout(()=>{
    newCard.classList.remove('in-right');
  ...
  document.querySelector('#card'+order[position]+' .qnum').textContent = cur + 1;
...
function submitTest(){ 
  ...
  if($('showAnswers') && $('showAnswers').checked){
