...
function capitalize(s){if(!s) return 'User'; return s.charAt(0).toUpperCase()+s.slice(1)}
...
$('themeToggle').addEventListener('click',()=>{
  light=!light;
  if(light){
    document.body.style.background='linear-gradient(180deg,#f7fbff, #e9f2ff)';
    document.body.style.color='#021023';
    document.documentElement.style.setProperty('--card','#ffffff');
    document.documentElement.style.setProperty('--text','#021023');
    $('themeToggle').innerText='Dark Mode';
  } else {
    document.body.style.background =
      'radial-gradient(circle at 12% 18%, rgba(57,255,20,0.045) 0%, rgba(57,255,20,0.03) 6%, transparent 20%),' +
      'radial-gradient(circle at 82% 78%, rgba(255,100,200,0.035) 0%, rgba(255,100,200,0.02) 6%, transparent 18%),' +
      'radial-gradient(circle at 50% 40%, rgba(0,200,255,0.03) 0%, rgba(0,200,255,0.02) 7%, transparent 22%),' +
      'linear-gradient(180deg,#000000 0%, #05050a 60%)';
    document.body.style.color='';
    document.documentElement.style.setProperty('--card','rgba(10,10,12,0.70)');
...
$('startBtn').addEventListener('click',()=>{
  ...
  if($('totalCount')) $('totalCount').innerText = questions.length;
  // render first question
  $('startPage').style.display='none';
  $('quiz').style.display='block';
  remainingSeconds = parseInt($('timerMinutes')?.value || '30')*60;
  if($('timerText')) $('timerText').innerText = formatTime(remainingSeconds);
  ...
  renderStage(); showQuestion(0);
});

// render stage
function renderStage(){
  const stage = $('qstage'); stage.innerHTML='';
  questions.forEach((q,i)=>{
    const card = document.createElement('div'); card.className='question-card'; card.id='card'+i;
    card.innerHTML = `<div class='qtext'><strong>Q${i+1}.</strong> ${q.q}</div>`;
    const opts = document.createElement('div'); opts.className='options';
    q.opts.forEach((o,j)=>{
      const letter=['A','B','C','D'][j];
      const opt = document.createElement('label'); opt.className='option';
      opt.innerHTML = `<input type='radio' name='q${i}' value='${letter}' /> <strong>${letter}.</strong> ${o}`;
      opt.addEventListener('click',()=>{
        const radios = document.querySelectorAll('input[name="q'+i+'"]');
        for (let r of radios) if (r.value===letter) r.checked = true;
    ...
    card.style.display='none'; stage.appendChild(card);
  });
  if($('answeredCount')) $('answeredCount').innerText = Object.keys(userAnswers).length;
}

// show question without overlap
function showQuestion(position){
  if(animating) return;
  animating=true;
  // hide all
  document.querySelectorAll('.question-card').forEach(c=>{
    c.classList.remove('show','out-left','out-right','in-left','in-right');
  ...
  // small animation: add in-right then remove, but ensure only one visible
  newCard.classList.add('in-right');
  setTimeout(()=>{ newCard.classList.remove('in-right'); newCard.classList.add('show'); animating=false; updateProgress(); }, 40);

  cur = position;
  if($('prevBtn')) $('prevBtn').style.display = (cur>0)?'inline-block':'none';
  if($('nextBtn')) $('nextBtn').innerText = (cur < questions.length-1)?'Next':'Submit';
...
// navigation
if($('nextBtn')) $('nextBtn').addEventListener('click',()=>{ saveCurrent(); if(cur<questions.length-1) showQuestion(cur+1); else submitTest(); });
if($('prevBtn')) $('prevBtn').addEventListener('click',()=>{ saveCurrent(); if(cur>0) showQuestion(cur-1); });

function saveCurrent(){ const sel = document.querySelector('input[name="q'+order[cur]+'"]:checked'); if(sel){ userAnswers[order[cur]] = sel.value; } if($('answeredCount')) $('answeredCount').innerText = Object.keys(userAnswers).length; }
function updateAnswered(){ if($('answeredCount')) $('answeredCount').innerText = Object.keys(userAnswers).length; }
function updateProgress(){ const pct = Math.round(((cur+1)/questions.length)*100); if($('progressBar')) $('progressBar').style.width = pct+'%'; if($('progressText')) $('progressText').innerText = `Question ${cur+1} / ${questions.length}`; }

function submitTest(){ if(timerInterval) clearInterval(timerInterval); saveCurrent(); let score=0; for(let i=0;i<questions.length;i++){ if(userAnswers[i]===questions[i].answer) score++; }
  if($('qstage')) $('qstage').style.display='none'; const controls = document.querySelector('.controls'); if(controls) controls.style.display='none'; if($('result')) $('result').style.display='block';
  if($('timerText')) $('timerText').style.display='none'; const pw = document.querySelector('.progress-wrap'); if(pw) pw.style.display='none';
  const name = $('username') ? $('username').value.trim() : 'User';
  if($('result')) $('result').innerHTML = `<div style="font-size:20px;font-weight:700">${capitalize(name)}, your score is ${score}/${questions.length}</div>`;
  if($('showAnswers') && $('showAnswers').checked && $('result')){
    const det = document.createElement('div'); det.style.marginTop='12px';
    questions.forEach((q,i)=>{ const wrap=document.createElement('div'); wrap.style.marginTop='8px';
      wrap.innerHTML=`<strong>Q${i+1}.</strong> ${q.q}`;
      q.opts.forEach((opt,j)=>{ const letter=['A','B','C','D'][j]; const line=document.createElement('div'); line.style.padding='8px'; line.style.borderRadius='6px';
        if(letter===q.answer) line.style.background='rgba(57,255,20,0.08)';
        if(userAnswers[i]===letter && letter!==q.answer) line.style.background='rgba(255,80,80,0.06)';
        line.innerHTML=`<strong>${letter}.</strong> ${opt}`; wrap.appendChild(line);
      });
...
// init small UI interactions
document.addEventListener('DOMContentLoaded',()=>{
  document.querySelectorAll('.ripple').forEach(btn=>{
    btn.addEventListener('click',function(e){
      const circle=document.createElement('span'); circle.style.position='absolute'; circle.style.borderRadius='50%';
      circle.style.width='120px'; circle.style.height='120px'; circle.style.left=(e.offsetX-60)+'px'; circle.style.top=(e.offsetY-60)+'px';
      circle.style.background='rgba(255,255,255,0.06)'; circle.style.transform='scale(0)'; circle.style.transition='transform .6s, opacity .8s';
      this.appendChild(circle); setTimeout(()=>{ circle.style.transform='scale(1)'; circle.style.opacity='0'; },10);
      setTimeout(()=>{ circle.remove(); },900);
    });
  });
  // ensure prev button hidden initially
