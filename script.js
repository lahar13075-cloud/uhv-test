...
// ------- SAFETY: ensure DOM is ready before wiring things -------
document.addEventListener('DOMContentLoaded', () => {
  // initialize total count display
  const total = questions.length;
  if ($('totalCount')) $('totalCount').innerText = total;

  // attach start / nav listeners (defensive checks)
  if ($('startBtn')) {
  ...
  document.querySelectorAll('.ripple').forEach(btn=>{
    btn.addEventListener('click', function(e){
      ...
      circle.style.left=(e.offsetX-60)+'px'; circle.style.top=(e.offsetY-60)+'px';
      circle.style.background='rgba(255,255,255,0.06)'; circle.style.transform='scale(0)';
      circle.style.transition='transform .6s, opacity .8s';
      this.appendChild(circle);
      setTimeout(()=>{ circle.style.transform='scale(1)'; circle.style.opacity='0'; },10);
...
function startTest(){
  ...
  // timer
  const minsInput = $('timerMinutes');
  const mins = (minsInput && parseInt(minsInput.value,10)) ? parseInt(minsInput.value,10) : 30;
...
function renderStage(){
  ...
  questions.forEach((q,i)=>{
    ...
    q.opts.forEach((o,j)=>{
      ...
      input.name = `q${i}`;
      input.value = letter;
      // set an id for input (not required but helpful)
      input.id = `q${i}_${letter}`;
      ...
      // click handler on label: check the radio & store answer
      label.addEventListener('click', ()=>{
        input.checked = true;
        userAnswers[i] = letter; // store by original index
        updateAnswered();
      });
...
function showQuestion(position){
  ...
  if(prev){
    const prevIndex = parseInt(prev.id.replace('card',''),10);
    const direction = (order.indexOf(prevIndex) < position) ? 'out-left' : 'out-right';
    prev.classList.add(direction);
    prev.classList.remove('show');
  }

  const incoming = prev ? ((order.indexOf(parseInt(prev.id.replace('card',''),10)) < position) ? 'in-right' : 'in-left') : 'in-right';
  ...
  setTimeout(()=>{
    ...
    // restore selection if user answered earlier
    const radios = document.getElementsByName('q' + order[position]);
    if (radios && radios.length){
      for (const r of radios){
        r.checked = (userAnswers[order[position]] === r.value);
      }
    }
...
function updateProgress(){
  ...
  // show progress as proportion of questions completed (use cur+1 for human readable percent)
  const pct = Math.round(((cur + 1) / total) * 100);
  if ($('progressBar')) $('progressBar').style.width = pct + '%';
  if ($('progressText')) $('progressText').innerText = `Question ${cur + 1} / ${total}`;
}
...
function submitTest(){
  ...
  // optionally show answers if checkbox is present & checked
  const showAnswersEl = $('showAnswers');
  if (showAnswersEl && showAnswersEl.checked && $('result')){
    ...
    questions.forEach((q, i) => {
      ...
      q.opts.forEach((opt, j) => {
        ...
        if (userAnswers[i] === letter && letter !== q.answer) line.style.background = 'rgba(255,80,80,0.1)';
